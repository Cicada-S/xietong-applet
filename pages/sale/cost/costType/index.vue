	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('costTypeAdd')" type="primary" size="small" @click="$next(`/pages/sale/cost/costType/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('costTypeEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('costTypeBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/costType/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delcostType,"POST",this.selectRows).then((val) => {
			...
			getCostTypeRes() {
				...
				this.$axios.request(this.$api.getCostTypeRes, "GET", params).then((val) => {
