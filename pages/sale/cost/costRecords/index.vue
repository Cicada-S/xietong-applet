<template>
	<view class="container" :style="{minHeight:$height()}">
		...
		<u-button v-if="$utils.hasPerm('costRecordsAdd')" type="primary" size="small" @click="$next(`/pages/sale/cost/costRecords/add`)" text="新增"></u-button>
		<u-button v-if="$utils.hasPerm('costRecordsEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
		<u-button v-if="$utils.hasPerm('costRecordsBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/sale/cost/costRecords/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delcostRecords,"POST",this.selectRows).then((val) => {
			...
			getCostRecordsRes() {
				...
				this.$axios.request(this.$api.getCostRecordsRes, "GET", params).then((val) => {
