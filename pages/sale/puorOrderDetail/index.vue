<template>
	<view class="container" :style="{minHeight:$height()}">
		...
		<u-button v-if="$utils.hasPerm('puorOrderDetailAdd')" type="primary" size="small" @click="$next(`/pages/sale/puorOrderDetail/add`)" text="新增"></u-button>
		...
		<u-button v-if="$utils.hasPerm('approvalErr')" type="primary" size="small" @click="approvalErr" text="弃审"></u-button>
			</view>
			...
			<u-checkbox-group v-else placement="column" @change="changeCheckBox">
				<view class="item" v-for="(item,index) in list" :key="index">
					<view class="order-no king-align__center king-flex__between">
						...
						<view class="right king-align__center">
							<u-checkbox :code="item.id"></u-checkbox>
						</view>
					</view>
					<u-line margin="20rpx 0px"></u-line>
					<view class="info">
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/puorOrderDetail/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delPuorOrderDetail,"POST",this.selectRows).then((val) => {
			...
			getPuorOrderDetailRes() {
				...
				this.$axios.request(this.$api.getPuorOrderDetailRes, "GET", params).then((val) => {
