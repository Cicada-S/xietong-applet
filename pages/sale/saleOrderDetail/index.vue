	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex" v-if="type == 0">
				<u-button v-if="$utils.hasPerm('saleOrderDetailAdd')" type="primary" size="small" @click="$next(`/pages/sale/saleOrderDetail/add?type=${type}`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('saleOrderDetailEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('saleOrderDetailBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
			<view class="search-item king-flex" v-if="type == 1">
				<u-button v-if="$utils.hasPerm('puorOrderDetailAdd')" type="primary" size="small" @click="$next(`/pages/sale/saleOrderDetail/add?type=${type}`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('puorOrderDetailEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('puorOrderDetailBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
	    	...
	    	edit(){
				...
				this.$axios.request(this.$api.getSaleOrderDetailRes,"GET",{
					id:this.list[idx].orderId
				}).then((val) => [
					this.$next(`/pages/mine/saleOrderDetail/add?result=${this.$utils.encryption(val.data)}`)
				])
			...
			del(){
				...
				this.$axios.request(this.$api.delSaleOrderDetail,"POST",params).then((val) => {
			...
			getSaleOrderDetailRes() {
				...
				this.$axios.request(this.$api.getSaleOrderDetailRes, "GET", params).then((val) => {
