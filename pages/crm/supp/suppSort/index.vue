  ...
  <view class="container" :style="{ minHeight: $height() }">
    <view class="search-box">
      <view class="search-item king-flex">
        <u-button
          v-if="$utils.hasPerm('cusSortAdd')"
          type="primary"
          size="small"
          @click="$next(`/pages/crm/cus/cusSort/add`)"
          text="新增"
        ></u-button>
        <u-button
          v-if="$utils.hasPerm('cusSortEdit')"
          type="error"
          size="small"
          @click="edit"
          text="编辑"
        ></u-button>
        <u-button
          v-if="$utils.hasPerm('cusSortBatchDelete')"
          type="primary"
          size="small"
          @click="del"
          text="删除"
...
export default {
  ...
  methods: {
    ...
    edit() {
      ...
      this.$next(
        `/pages/test/cusSort/add?result=${this.$utils.encryption(
          this.selectRows[0]
        )}`
      )
    },
    del() {
      ...
      this.$axios
        .request(this.$api.delcusSort, "POST", this.selectRows)
        .then((val) => {
    ...
    getcusSortRes() {
      ...
      this.$axios
        .request(this.$api.getCusSortRes, "GET", params)
        .then((val) => {
