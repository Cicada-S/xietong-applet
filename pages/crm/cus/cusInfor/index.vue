	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('cusInforAdd')" type="primary" size="small" @click="$next(`/pages/crm/cus/cusInfor/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('cusInforEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('cusInforBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/cusInfor/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delcusInfor,"POST",this.selectRows).then((val) => {
			...
			getcusInforRes() {
				...
				this.$axios.request(this.$api.getcusInforRes, "GET", params).then((val) => {
