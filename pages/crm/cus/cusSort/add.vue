	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="box">
			<u-form labelPosition="right" labelWidth="130" ref="uForm">
				...
				<u-form-item label="客户类型名称" borderBottom border :required="true">
					<u-input v-model="formData.name" placeholder="请填写客户类型名称" border="none"></u-input>
				</u-form-item>
	...
	export default {
		...
		onLoad(options) {
			...
			uni.setNavigationBarTitle({
				title:`${this.formData.id ? '编辑' : '新增'}客户类型`
			})
		...
		methods: {
			...
			submit(){
				let url = this.$api.addCusSort
				if(this.formData.id){
					url = this.$api.editCusSort
				}
