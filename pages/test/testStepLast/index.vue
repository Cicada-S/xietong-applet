	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testStepLastAdd')" type="primary" size="small" @click="$next(`/pages/test/testStepLast/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testStepLastEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testStepLastBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
	...
	export default {
		...
		methods: {
			...
			del(){
				...
				this.$axios.request(this.$api.deltestStepLast,"POST",this.selectRows).then((val) => {
			...
			getTestStepLastRes() {
				...
				this.$axios.request(this.$api.getTestStepLastRes, "GET", params).then((val) => {
