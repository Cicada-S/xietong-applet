	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testFormulaAdd')" type="primary" size="small" @click="$next(`/pages/test/testFormula/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testFormulaEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testFormulaBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/testFormula/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.deltestFormula,"POST",this.selectRows).then((val) => {
			...
			getTestFormulaRes() {
				...
				this.$axios.request(this.$api.getTestFormulaRes, "GET", params).then((val) => {
