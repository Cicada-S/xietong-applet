	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="box">
			<u-form labelPosition="right" labelWidth="100" ref="uForm">
				...
				<u-form-item label="标准名称" borderBottom border :required="true">
					<u-input v-model="formData.name" placeholder="请填写标准名称" border="none"></u-input>
				</u-form-item>
	...
	export default {
		...
		onLoad(options) {
			...
			uni.setNavigationBarTitle({
				title:`${this.formData.id ? '编辑' : '新增'}检验标准`
			})
		...
		methods: {
			...
			submit(){
				let url = this.$api.addtestStandard
				if(this.formData.id){
					url = this.$api.edittestStandard
				}
