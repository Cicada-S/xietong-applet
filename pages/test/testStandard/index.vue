	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testStandardAdd')" type="primary" size="small" @click="$next(`/pages/test/testStandard/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testStandardEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testStandardBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/testStandard/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.deltestStandard,"POST",this.selectRows).then((val) => {
			...
			getTestStandardRes() {
				...
				this.$axios.request(this.$api.getTestStandardRes, "GET", params).then((val) => {
