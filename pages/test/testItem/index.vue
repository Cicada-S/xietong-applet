	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testItemAdd')" type="primary" size="small" @click="$next(`/pages/test/testItem/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testItemEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testItemBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/testItem/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.deltestItem,"POST",this.selectRows).then((val) => {
			...
			getTestItemRes() {
				...
				this.$axios.request(this.$api.getTestItemRes, "GET", params).then((val) => {
