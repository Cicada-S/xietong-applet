	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testTypeAdd')" type="primary" size="small" @click="$next(`/pages/test/testType/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testTypeEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testTypeBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/testType/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.deltestType,"POST",this.selectRows).then((val) => {
			...
			getTestTypeRes() {
				...
				this.$axios.request(this.$api.getTestTypeRes, "GET", params).then((val) => {
