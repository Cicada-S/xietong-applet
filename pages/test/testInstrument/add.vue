    ...
    <view class="box">
      <u-form labelPosition="right" labelWidth="100" ref="uForm">
        ...
        <u-form-item label="下次校准时间" borderBottom border :required="true">
          <u-input
            readonly
            v-model="formData.nextCalibrationTime"
          ...
          >
            <view slot="suffix">
              <u-button
                type="primary"
                @click="chooseTime('nextCalibrationTime')"
                size="mini"
                text="请选择"
              ></u-button>
            </view>
  ...
  methods: {
    submit() {
      const url = this.formData.id
        ? this.$api.ediTtestInstrument
        : this.$api.addTestInstrument
