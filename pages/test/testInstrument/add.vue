	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="box">
			<u-form labelPosition="right" labelWidth="100" ref="uForm">
				...
				<u-form-item label="仪器名称" borderBottom border :required="true">
					<u-input v-model="formData.name" placeholder="请填写仪器名称" border="none"></u-input>
				</u-form-item>
				<u-form-item label="购买时间" borderBottom border :required="true">
					<u-input readonly v-model="formData.buyTime" placeholder="请选择购买时间" border="none">
						<view slot="suffix">
							<u-button type="primary" @click="chooseTime('buyTime')" size="mini" text="请选择"></u-button>
						</view>
					</u-input>
				</u-form-item>
				<u-form-item label="投入试用时间" borderBottom border :required="true">
					<u-input readonly v-model="formData.usedTime" placeholder="请选择投入使用时间" border="none">
						<view slot="suffix">
							<u-button type="primary" @click="chooseTime('usedTime')" size="mini" text="请选择"></u-button>
						</view>
					</u-input>
				</u-form-item>
				<u-form-item label="下次校准时间" borderBottom border :required="true">
					<u-input readonly v-model="formData.testTime" placeholder="请选择下次校准时间" border="none">
						<view slot="suffix">
							<u-button type="primary" @click="chooseTime('testTime')" size="mini" text="请选择"></u-button>
						</view>
					</u-input>
                </u-form-item>
	...
	export default {
		...
		onLoad(options) {
			...
			uni.setNavigationBarTitle({
				title:`${this.formData.id ? '编辑' : '新增'}检验仪器`
			})
			
		},
		methods: {
			submit(){
				let url = this.$api.addtestInstrument
				if(this.formData.id){
					url = this.$api.edittestInstrument
				}
