	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testInstrumentAdd')" type="primary" size="small" @click="$next(`/pages/test/testInstrument/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testInstrumentEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testInstrumentBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/testInstrument/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.deltestInstrument,"POST",this.selectRows).then((val) => {
			...
			getTestInstrumentRes() {
				...
				this.$axios.request(this.$api.getTestInstrumentRes, "GET", params).then((val) => {
