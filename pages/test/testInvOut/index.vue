	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testInvOutAdd')" type="primary" size="small" @click="$next(`/pages/test/testInvOut/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testInvOutEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testInvOutBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/testInvOut/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.deltestInvOut,"POST",this.selectRows).then((val) => {
			...
			getTestInvOutRes() {
				...
				this.$axios.request(this.$api.getTestInvOutRes, "GET", params).then((val) => {
