	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testInvInAdd')" type="primary" size="small" @click="$next(`/pages/test/testInvIn/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testInvInEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testInvInBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/testInvIn/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.deltestInvIn,"POST",this.selectRows).then((val) => {
			...
			getTestInvInRes() {
				...
				this.$axios.request(this.$api.getTestInvInRes, "GET", params).then((val) => {
