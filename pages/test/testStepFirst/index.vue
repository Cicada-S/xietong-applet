	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('testStepFirstAdd')" type="primary" size="small" @click="$next(`/pages/test/testStepFirst/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('testStepFirstEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('testStepFirstBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/testStepFirst/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.deltestStepFirst,"POST",this.selectRows).then((val) => {
			...
			getTestStepFirstRes() {
				...
				this.$axios.request(this.$api.getTestStepFirstRes, "GET", params).then((val) => {
