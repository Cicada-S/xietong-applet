  ...
  <view class="container" :style="{ minHeight: $height() }">
    <view class="box">
      <u-form labelPosition="right" labelWidth="100" ref="uForm">
        ...
        <dynamicItem
          :value="formData.extJson"
          ref="dynamicRef"
          :association="formData.workStepId"
            ...
            <u-table align="left">
              ...
              <u-checkbox-group placement="column" @change="changeCheckBox">
                <u-tr v-for="(item, index) in formData.badList" :key="index">
                  ...
                  /></u-td>
                  <u-td style="width: 20%"
                    ><input v-model="item.badNum" type="text"
                  /></u-td>
                  <u-td style="width: 20%"
                    ><input v-model="item.badgrade" type="text"
                  /></u-td>
                  <u-td style="width: 20%"
                    ><input v-model="item.badway" type="text"
                  /></u-td>
                  <u-td style="width: 20%"
    ...
    <u-popup
      :show="showtestFormula"
      mode="bottom"
      @close="() => (showtestFormula = false)"
      round="10"
      :closeable="true"
    >
      <view class="modal-box">
        <scroll-view
          ...
          style="height: 100%"
          scroll-y="auto"
          @scrolltolower="() => this.$refs.testFormula.loadMore()"
        >
    ...
    >
      <view class="modal-box">
        <scroll-view
          ...
          style="height: 100%"
          scroll-y="auto"
          @scrolltolower="() => this.$refs.mulBadItem.loadMore()"
        >
...
export default {
  ...
  onLoad(options) {
    if (options.result) {
      let pageOptions = this.$utils.decrypt(options.result);
      this.formData = pageOptions;
      this.formData.taskId = pageOptions.workStepId;
      console.log(this.$utils.decrypt(options.result), this.formData);
      this.load();
    }
  ...
  methods: {
    ...
    submit() {
      let url = this.$api.addTestStepFirst;
      if (this.formData.id) {
        url = this.$api.editTestStepFirst;
      }
