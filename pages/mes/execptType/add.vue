		...
		<view class="box">
			<u-form labelPosition="right" labelWidth="130" ref="uForm">
				...
				<u-form-item label="类型名称" borderBottom border :required="true">
					<u-input v-model="formData.name" placeholder="请填写类型名称" border="none"></u-input>
				</u-form-item>
				<u-form-item label="警灯位置" borderBottom border :required="true">
					<u-input v-model="formData.warningLocation" placeholder="请填写类型名称" border="none"></u-input>
				</u-form-item>
				<u-form-item label="状态" borderBottom border>
					<u-radio-group v-model="formData.taskStatus" placement="row">
						<u-radio name="0" label="正常"></u-radio>
						<u-radio name="1" label="停用" :customStyle="{marginLeft:'10rpx'}"></u-radio>
					</u-radio-group>
				</u-form-item>
	...
	export default {
		...
		onLoad(options) {
			...
			uni.setNavigationBarTitle({
				title:`${this.formData.id ? '编辑' : '新增'}上报类型`
			})
