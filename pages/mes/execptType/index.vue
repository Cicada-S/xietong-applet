	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('execptTypeAdd')" type="primary" size="small" @click="$next(`/pages/mes/execptType/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('execptTypeEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('execptTypeBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
				...
				<view class="info">
					<view class="king-flex king-flex__column">
						...
						<view class="king-align__center info-item">
							<text>警灯位置：</text>
							<text>{{item.warningLocation}}</text>
						</view>
						<view class="king-align__center info-item">
							<text>状态：</text>
							<u-tag size="error" v-if="item.taskStatus == 0" text="停用"></u-tag>
							<u-tag type="success" size="mini" v-else text="正常"></u-tag>
						</view>
						<view class="king-align__center info-item">
							<text>备注：</text>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/execptType/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delexecptType,"POST",this.selectRows).then((val) => {
			...
			getExecptTypeRes() {
				...
				this.$axios.request(this.$api.getExecptTypeRes, "GET", params).then((val) => {
