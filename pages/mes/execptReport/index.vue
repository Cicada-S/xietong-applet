<template>
	<view class="container" :style="{minHeight:$height()}">
		...
		<u-button v-if="$utils.hasPerm('execptReportAdd')" type="primary" size="small" @click="$next(`/pages/sale/execptReport/add`)" text="新增"></u-button>
		<u-button v-if="$utils.hasPerm('execptReportEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
		<u-button v-if="$utils.hasPerm('execptReportBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
				...
				<view class="item" v-for="(item,index) in list" :key="index">
					<view class="order-no king-align__center king-flex__between">
						<view class="left king-align__center">
							...
							<text>类型名称：{{item.execptTypeName}}</text>
						</view>
						<view class="right king-align__center">
							<u-radio :code="item.execptId"></u-radio>
						</view>
					</view>
					...
					<view class="info">
						<view class="king-flex king-flex__column">
                                        <view class="king-align__center info-item">
											<text>一级上报时长：</text>
											<text>{{item.firstLength}}秒</text>
										</view>
										<view class="king-align__center info-item">
											<text>一级上报人：</text>
											<text>{{item.firstReportName||''}}</text>
										</view>
										
										<view class="king-align__center info-item">
											<text>二级上报时长：</text>
											<text>{{item.secondLength}}秒</text>
										</view>
										<view class="king-align__center info-item">
											<text>二级上报人：</text>
											<text>{{item.secondReportName||''}}</text>
										</view>
										
										<view class="king-align__center info-item">
											<text>三级上报时长：</text>
											<text>{{item.thridLength}}秒</text>
										</view>
										<view class="king-align__center info-item">
											<text>三级上报人：</text>
											<text>{{item.thridReportName||''}}</text>
										</view>
			...
			<u-checkbox-group v-else placement="column" @change="changeCheckBox">
				<view class="item" v-for="(item,index) in list" :key="index">
					<view class="order-no king-align__center king-flex__between">
						...
						<view class="right king-align__center">
							<u-checkbox :code="item.execptTypeIs"></u-checkbox>
						</view>
					</view>
					<u-line margin="20rpx 0px"></u-line>
					<view class="info">
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/ExecptReport/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delExecptReport,"POST",this.selectRows).then((val) => {
			...
			getExecptReportRes() {
				...
				this.$axios.request(this.$api.getExecptReportRes, "GET", params).then((val) => {
