	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('groupStepAdd')" type="primary" size="small" @click="$next(`/pages/mes/groupStep/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('groupStepEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('groupStepBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/groupStep/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delGroupStep,"POST",this.selectRows).then((val) => {
			...
			getGroupStepRes() {
				...
				this.$axios.request(this.$api.getGroupStepRes, "GET", params).then((val) => {
