	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('proQualityAdd')" type="primary" size="small" @click="$next(`/pages/mes/quality/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('proQualityEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('proQualityBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/quality/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delProQuality,"POST",this.selectRows).then((val) => {
			...
			getProQualityRes() {
				...
				this.$axios.request(this.$api.getProQualityRes, "GET", params).then((val) => {
