	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('proPlanCategoryAdd')" type="primary" size="small" @click="$next(`/pages/mes/proPlanCategory/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('proPlanCategoryEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('proPlanCategoryBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/test/proPlanCategory/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delProPlanCategory,"POST",this.selectRows).then((val) => {
			...
			getProPlanCategoryRes() {
				...
				this.$axios.request(this.$api.getProPlanCategoryRes, "GET", params).then((val) => {
