	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('badItemAdd')" type="primary" size="small" @click="$next(`/pages/mes/badItem/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('badItemEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('badItemBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/badItem/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delBadItem,"POST",this.selectRows).then((val) => {
			...
			getBadItemRes() {
				...
				this.$axios.request(this.$api.getBadItemRes, "GET", params).then((val) => {
