	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('mesUnitAdd')" type="primary" size="small" @click="$next(`/pages/mes/unit/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('mesUnitEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('mesUnitBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
			
			<u-search placeholder="请输入计量单位名称" :clearabled="true" :showAction="false" @search="search" @clear="clear" v-model="formData.name" :customStyle="{margin:'10rpx 10rpx'}"></u-search>
	...
	export default {
		...
		methods: {
			...
			getUnitRes() {
				...
				this.$axios.request(this.$api.getUnitRes, "GET", params).then((val) => {
