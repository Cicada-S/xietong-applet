	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('proPlanAdd')" type="primary" size="small" @click="$next(`/pages/mes/proPlan/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('proPlanEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('proPlanBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view> 
			
			<u-search placeholder="请输入计划单号" :clearabled="true" :showAction="false" @search="search" @clear="clear" v-model="formData.code" :customStyle="{margin:'10rpx 10rpx'}"></u-search>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/proPlan/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delProPlan,"POST",this.selectRows).then((val) => {
			...
			getProPlanRes() {
				...
				this.$axios.request(this.$api.getProPlanPage, "GET", params).then((val) => {
