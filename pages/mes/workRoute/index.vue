	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('workRouteAdd')" type="primary" size="small" @click="$next(`/pages/mes/workRoute/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('workRouteEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('workRouteBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
			...
			<u-radio-group v-if="type == 'radio'" placement="column" @change="checkRadio">
				<view class="item" v-for="(item,index) in list" :key="index">
					...
					<view class="footer king-align__center king-flex__between">
						...
						<u-button @click="submit" :customStyle="{width:'80%',position:'fixed',left:'10%',bottom:'20px',zIndex:'999'}" type="primary" text="确定">
							
						<!--</u-button> <u-radio :name="item.id"></u-radio>		
						<u-button @click="submit" :customStyle="{width:'80%',position:'fixed',left:'10%',bottom:'20px',zIndex:'999'}" type="primary" text="确定"> -->
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/workRoute/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			del(){
				...
				this.$axios.request(this.$api.delWorkRoute,"POST",this.selectRows).then((val) => {
			...
			getWorkRouteRes() {
				...
				this.$axios.request(this.$api.getWorkRouteRes, "GET", params).then((val) => {
