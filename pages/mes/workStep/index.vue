	...
	<view class="container" :style="{minHeight:$height()}">
		<view class="search-box">
			<view class="search-item king-flex">
				<u-button v-if="$utils.hasPerm('workStepAdd')" type="primary" size="small" @click="$next(`/pages/mes/workStep/add`)" text="新增"></u-button>
				<u-button v-if="$utils.hasPerm('workStepEdit')" type="error" size="small" @click="edit" text="编辑"></u-button>
				<u-button v-if="$utils.hasPerm('workStepBatchDelete')" type="primary" size="small" @click="del" text="删除"></u-button>
			</view>
				...
				<view class="item" v-for="(item,index) in list" :key="index">
					...
					<view class="info">
						<view class="king-flex king-flex__column">
							...
							<view class="king-align__center info-item">
								<text>不良品项：</text>
								<text>{{item.mulBadItem || ''}}</text>
							</view>
							<view class="king-align__center info-item">
								<text>加工类型：</text>
								<text>{{item.processingTypeName || ''}}</text>
							</view>
							<view class="king-align__center info-item">
								<text>负责人：</text>
							...
							<view class="king-align__center info-item">
								<text>报工人：</text>
								<text>{{item.reportRightName || ''}}</text>
							</view>
							<view class="king-align__center info-item">
								<text>排序：</text>
								<text>{{item.sortCode || ''}}</text>
							</view>
							<view class="king-align__center info-item">
								<text>备注：</text>
	...
	export default {
		...
		methods: {
			...
			edit(){
				...
				this.$next(`/pages/mes/workStep/add?result=${this.$utils.encryption(this.selectRows[0])}`)
			...
			getWorkStepRes() {
				...
				this.$axios.request(this.$api.getWorkStepRes, "GET", params).then((val) => {
