...
<script>
	...
	export default{
		...
		onLoad(option) {
			...
			// 	this.commentList.value = data.commentList || []
			// 	// 当前节点，里面是增加了相关记录
			// 	const childNode = workFlowUtils.getChildNode(JSON.parse(data?.initiatorModelJson), option?.doneActivityId, data?.commentList)
			// 	if (data?.formType === 'DESIGN') {
			// 		if (data.initiatorFormJson && data.initiatorDataJson) {
			// 			const formJson = JSON.parse(data.initiatorFormJson)
			// 			if (formJson && formJson.list) {
			// 				const obj = workFlowUtils.handleSnowyFormJsonList(formJson.list, true)
			// 				fieldConfigs.value = obj.fieldConfigs
			// 				const initData = JSON.parse(data.initiatorDataJson)
			// 				if (initData && initData[obj.topTableName]) {
			// 					this.formBuildData = initData[obj.topTableName]
			// 				}
			// 			}
			...
			// 		this.customFromCode = childNode.properties.formInfo.find((f) => f.key === 'MOBILE_FORM_URL').value
			// 		// 表单数据
			// 		this.initiatorDataJson = JSON.parse(data?.initiatorDataJson)
			// 	}
			// })
		},
		methods:{
